...
class Mscoco(data.Dataset):
    def __init__(self, train=True, sigma=1,
        ...
        self.img_folder = '/media/ubuntu/文档/code/torch/AlphaPose-pytorch/train_sppe/data/coco/images/train2017'  # root image folders
        ...
        with h5py.File('../data/coco/annot_coco.h5', 'r') as annot:
            self.imgname_coco_train = annot['imgname'][:2000]
            self.bndbox_coco_train = annot['bndbox'][:2000]
            self.part_coco_train = annot['part'][:2000]
            # val
            self.imgname_coco_val = annot['imgname'][0:20]
            self.bndbox_coco_val = annot['bndbox'][0:20]
            self.part_coco_val = annot['part'][0:20]
